...
class LoadController extends ConsoleController
{
    ...
    {
        //print_r($this->fs->getProduct($id));die;
    ...
    public function actionDiffSupplier(){
        ...
        foreach ($res2 as $supplier_id) {
            ...
            if($sup){
                ...
                $dels = Product::deleteAll(['supplier_id'=>$sup->id]);
            }
    ...
    public function actionFix(){
        $products = Product::find()->offset(50000)->limit(10000)->all();
        foreach ($products as $product){
            $box = $product->eav_kolicestvo_v_asike;
            if($box){
                $product->price = $product->price / $box->value;
                $product->price_purchase = $product->price_purchase / $box->value;
                $product->discount = $product->discount / $box->value;
                $product->in_box = $box->value;
                $product->save(false);
            }
        }
